...
// create 函式負責處理 POST 請求以創建新產品。它將從 req.file.path 中取得檔案路徑，並將其存入 req.body.image 中，接著使用 Product.create() 方法創建新的產品資料。如果成功創建，將返回狀態碼 200 和成功訊息，否則根據不同的錯誤類型回傳適當的錯誤訊息。
...
// getAll 函式負責處理 GET 請求以獲取所有產品的資料，接收多個查詢參數（如 sortBy、sortOrder、itemsPerPage、page 和 search），並根據這些參數從資料庫中檢索符合條件的產品資料。搜尋條件包括產品名稱和描述的文本搜尋。取得資料後，回傳結果包括分頁的產品資料和總數。
export const getAll = async (req, res) => {
  try {
    ...
    const total = await Product.estimatedDocumentCount() // 取得產品總數
    res.status(StatusCodes.OK).json({
      success: true,
      message: '',
      result: {
